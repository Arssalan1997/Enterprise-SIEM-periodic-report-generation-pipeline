...
with st.form(key="quarterly_report_file_upload"):

    st.markdown(
        "<h1 style='color:green; text-align:center; font-family:sans-serif;'>Upload Last Season Report File</h1>",
        unsafe_allow_html=True
                )

    st.session_state.quarterly_report_file = st.file_uploader("no need", label_visibility="hidden", type=["xlsx"])
...
if next_button:
    if st.session_state.quarterly_report_file is None:
        st.write("\n")
