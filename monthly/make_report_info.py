...
def make_sheet_names_with_messages_that_have_zero_as_aggregation_value_for_sheets_that_have_aggregation_column\
                (sheet_name_as_key__and__messages_with_zero_for_aggregation_number_as_value):

    # same as the comment of first code line in 'make_sheet_names_with_issues_in_weekly_report_files()' function of
    # this module
    st.session_state.df__messages_with_zero_as_aggregation_value_in_some_worksheets = pd.DataFrame({
        "SheetName": sheet_name_as_key__and__messages_with_zero_for_aggregation_number_as_value.keys(),
        "Messages With \'0\' as Aggregation Value": sheet_name_as_key__and__messages_with_zero_for_aggregation_number_as_value.values()
    })
...
def make_sheet_names_with_issues_in_weekly_report_files(filename_as_key__and__sheet_names_with_issue_as_value):
    ...
    st.session_state.df__sheet_names_with_issues_in_weekly_report_files = pd.DataFrame({
        'Weekly Report Filename': filename_as_key__and__sheet_names_with_issue_as_value.keys(),
        'SheetNames With Issues': filename_as_key__and__sheet_names_with_issue_as_value.values()
    })

    # makes row indexing start from '0'(instead of '1')
    st.session_state.df__sheet_names_with_issues_in_weekly_report_files.index += 1
...
def make_sheet_names_not_found_in_mapping_file(filename_as_key_and_sheet_names_in_file_but_not_in_mapping_file_as_value):
    ...
    'display_sheet_names_that_include_in_at_least_one_of_weekly_report_files__but__exclude_in_corresponding_mapping_file'
    """

    # same as the comment of first code line in 'make_sheet_names_with_issues_in_weekly_report_files()' function of
    # this module
    st.session_state.df__sheet_names_not_found_in_mapping_file = pd.DataFrame({
        'Weekly Report Filename': filename_as_key_and_sheet_names_in_file_but_not_in_mapping_file_as_value.keys(),
